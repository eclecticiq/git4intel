...
def create_user():
    ...
    org_loc_id = "location--3fc1c688-c7e9-4609-ac72-01ac8b4afbc1"
    user_id = stix2.v21.Identity(id=get_deterministic_uuid(prefix="identity--", seed=new_username),
                                 identity_class='individual', name=new_username, sectors=[slugify("IT Consulting & Other Services")])
    org_id = stix2.v21.Identity(id=get_deterministic_uuid(prefix="identity--", seed=new_org_name),
                                identity_class='organization', name=new_org_name, sectors=[slugify("IT Consulting & Other Services")])
    org_rel = stix2.v21.Relationship(id=get_deterministic_uuid(prefix="relationship--", seed=(str(user_id.id) + str(
        org_id.id) + 'relates_to')), source_ref=user_id.id, target_ref=org_id.id, relationship_type='relates_to')
    loc_rel = stix2.v21.Relationship(id=get_deterministic_uuid(prefix="relationship--", seed=(str(
        org_id.id) + org_loc_id + 'located_at')), source_ref=org_id.id, target_ref=org_loc_id, relationship_type='located_at')
...
def main():
    ...
    # Register the user:
    id_responses = g4i.register_user(id_bundle)
    print(id_responses)
