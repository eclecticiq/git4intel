...
def feed_handler(url, source_type, username=None, password=None, headers=None, parameters=None):
	if source_type == 'linefile':
		return LineFeed(url, username, password)
	elif source_type == 'rss':
		return RSSFeed(url, username, password)
	elif source_type == 'jsonapi':
		return AlienVaultFeed(url, headers, parameters)
	else:
...
class FeedFile():
	def __init__(self, url, username=None, password=None, headers=None, parameters=None):
		if username is None and password is None:
			self.r = requests.get(url, stream=True, headers=headers, params=parameters)
		else:
			self.r = requests.get(url, stream=True, auth=HTTPBasicAuth(username, password), headers=headers, params=parameters)
...
class AlienVaultFeed(FeedFile):
	def __init__(self, url, headers=None, parameters=None):
		FeedFile.__init__(self, url, headers=headers, parameters=parameters)
		json_data = self.r.json()
		self.json = json_data['results']
